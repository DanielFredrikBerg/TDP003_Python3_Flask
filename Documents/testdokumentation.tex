...
\usepackage{caption}
\captionsetup[lstlisting]{font={normal,tt}}
...
För att uppfylla kraven om korrekt JSON, UTF-8 och Jinja2 i projektet har validerare skrivits i python av Daniel Huber. Frågor hänvisas till densamme. JSON valideras av JSON\_tester.py, UTF-8 valideras av utf-8\_tester.py och Jinja2 valideras av jinja2\_validator.py . Även ett program som kontrollerar att alla programfiler skrivits på engelska har skrivits. Koddokumentation återfinns i mappen \texttt{/tdp003/Documents/validation\_documentation/}  
...
\begin{itemize}
\item[]\textbf{Indata} Lägg till ett projekt med ett till sökfält, 'new\_search\_field' i data.json. Se Appendix \ref{json}. Testa sedan att köra hemsidan med flask run. Sök på 'new' med sökfältet 'new\_search\_list' markerat i /list.
\item[]\textbf{Resultat} Bara det tillagda projektet ges som sökresultat. 
\end{itemize}
...
\begin{itemize}
\item[]\textbf{Indata} Lägg till kommentarer från användare från klassens portfolio presentation.
\item[]\textbf{Resultat} Se Figur \ref{fig:10} i Appendix. 
\end{itemize}
...
\newpage
\appendix
\section{Data.json\label{json}}
\lstinputlisting[language=JSON, caption=Utskrift av innehållet i filen data.json]{data.json}
\newpage
\section{Feedback - Presentation av Portfolio}
\begin{figure}[h!]
  \caption{Inscannade dokumentet med feed-back från portfolio presentationen.\label{fig:10}}
  \centerline{\includegraphics[width=\textwidth, height=16cm]{../Pictures/presentation-feedback.jpg}}
\end{figure}
